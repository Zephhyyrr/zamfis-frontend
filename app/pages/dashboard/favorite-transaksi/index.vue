<template>
  <div>
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
      <div>
        <h1 class="text-2xl font-bold text-gray-800">Favorit Transaksi</h1>
        <p class="text-gray-600">Kelola uraian transaksi yang sering digunakan</p>
      </div>
    ...
    <div v-if="showUndoBanner"
      ...
      <p class="text-sm text-amber-800">Favorit transaksi dipindahkan ke draft. Ingin memulihkan sekarang?</p>
      <button type="button"
        class="inline-flex items-center rounded-lg bg-amber-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-amber-700 transition-colors disabled:opacity-60"
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Uraian Favorit</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr v-else v-for="(item, index) in filteredList" :key="item.id" class="hover:bg-gray-50">
              <td class="px-6 py-4 text-sm text-gray-500">{{ ((activeTab === 'active' ? activeParams.page : draftParams.page) - 1) * 10 + Number(index) + 1 }}</td>
    ...
    <FeaturesFavoriteTransaksiDeleteModal v-model="showDeleteModal" :item="selectedItem" :mode="deleteMode" @success="handleSuccess" />
