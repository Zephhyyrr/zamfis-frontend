<template>
  <div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
      <form @submit.prevent="submitForm">
        <div class="space-y-6">
          ...
          <div>
            ...
            <select v-model="form.jenis" required :disabled="isSubmitting"
              ...
              <option value="imsakiyah" :disabled="existingImsakiyah">
                Info Imsakiyah <span v-if="existingImsakiyah">(Hanya bisa 1)</span>
              </option>
              <option value="sejarah" :disabled="existingSejarah">
                Sejarah <span v-if="existingSejarah">(Hanya bisa 1)</span>
              </option>
            </select>
          </div>
        ...
        <div class="mt-8 flex justify-end items-center gap-3">
          ...
          <BaseButton type="submit" :isLoading="isSubmitting" text="Simpan Konten" icon="lucide:save"
            :fullWidth="false" />
        </div>
      </form>
    </div>
    ...
    <BaseModal v-model="showSuccessModal" title="Berhasil" icon="lucide:badge-check" type="success"
      confirmText="Kembali ke daftar konten" @confirm="handleSuccessConfirm">
...
const existingPengurus = ref(false);
const existingImsakiyah = ref(false);
const existingSejarah = ref(false);

onMounted(async () => {
  try {
    const [activeRes, draftRes] = await Promise.all([
      ContentService.getAllContents({ limit: 100 } as any),
      ContentService.getDraftContents({ limit: 100 } as any)
    ]);

    const activeData = (activeRes as any).data;
    const draftData = (draftRes as any).data;

    let allItems: any[] = [];

    if (Array.isArray(activeData)) {
      allItems = allItems.concat(activeData);
    } else if (activeData && activeData.data) {
      allItems = allItems.concat(activeData.data);
    }

    if (Array.isArray(draftData)) {
      allItems = allItems.concat(draftData);
    } else if (draftData && draftData.data) {
      allItems = allItems.concat(draftData.data);
    }

    existingPengurus.value = allItems.some((c: any) => c.jenis === 'pengurus');
    existingImsakiyah.value = allItems.some((c: any) => c.jenis === 'imsakiyah');
    existingSejarah.value = allItems.some((c: any) => c.jenis === 'sejarah');
  } catch (e) {
    console.error('Error checking existing content:', e);
  }
});
