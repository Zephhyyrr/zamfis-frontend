  ...
  <div class="min-h-screen bg-emerald-50 dark:bg-gray-900 transition-colors duration-300">
    ...
    <main class="container mx-auto px-4 md:px-12 py-10 md:py-16 max-w-4xl">
      ...
      <article v-else class="max-w-4xl mx-auto py-4">
        ...
        <div v-if="mediaItems.length > 0" class="w-full aspect-video rounded-2xl overflow-hidden mb-8 shadow-lg bg-gray-100 dark:bg-gray-800 relative group">
          <div class="flex transition-transform duration-500 h-full" :style="{ transform: `translateX(-${currentMediaIndex * 100}%)` }">
            <div v-for="(media, idx) in mediaItems" :key="idx" class="w-full h-full shrink-0 flex items-center justify-center bg-gray-100 dark:bg-gray-800 relative">
              <video v-if="media.type === 'video'" controls class="w-full h-full bg-black">
                <source :src="resolveAssetUrl(media.url)" type="video/mp4" />
                Your browser does not support the video tag.
              </video>
              <img v-else :src="resolveAssetUrl(media.url)" :alt="item.judul" class="w-full h-full object-cover">
            </div>
          </div>
          <div v-if="mediaItems.length > 1" class="absolute inset-0 flex items-center justify-between p-4 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
            <button @click="prevMedia" class="pointer-events-auto bg-black/50 text-white rounded-full p-2 hover:bg-black/70 transition">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
              </svg>
            </button>
            <button @click="nextMedia" class="pointer-events-auto bg-black/50 text-white rounded-full p-2 hover:bg-black/70 transition">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
              </svg>
            </button>
          </div>
          <div v-if="mediaItems.length > 1" class="absolute bottom-4 left-0 right-0 flex justify-center gap-2 z-10">
            <span v-for="(media, idx) in mediaItems" :key="'dot-'+idx" class="w-2.5 h-2.5 rounded-full transition-colors cursor-pointer" :class="idx === currentMediaIndex ? 'bg-white' : 'bg-white/50'" @click="currentMediaIndex = idx"></span>
          </div>
        </div>
        
        <div v-else class="w-full aspect-video rounded-2xl overflow-hidden mb-8 shadow-lg bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
...
const mediaItems = computed(() => {
  ...
  if (item.value.gambarUrl && item.value.gambarUrl.length > 0) {
    item.value.gambarUrl.forEach((url: string) => {
      items.push({ type: 'image', url });
    });
  }
  if (item.value.videoUrl) {
    items.push({ type: 'video', url: item.value.videoUrl });
  }
  return items;
});
