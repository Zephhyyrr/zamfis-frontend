<template>
  <section id="pengurus" class="scroll-mt-24">
    ...
    <div v-else-if="pengurusList.length === 0"
      class="text-center text-emerald-100/90 glass-card-dark p-10 rounded-2xl mx-auto max-w-md">
      Data pengurus belum tersedia.
    </div>
    <div v-else class="flex justify-center">
      <div class="relative max-w-4xl w-full group mx-auto">
        <div class="absolute -inset-2 rounded-[2rem] opacity-40 group-hover:opacity-60 transition-opacity duration-500"
          style="background: radial-gradient(ellipse at center, rgba(212,168,67,0.4) 0%, rgba(16,185,129,0.2) 60%, transparent 100%);">
        </div>
        <div
          class="relative glass-card rounded-3xl overflow-hidden hover:-translate-y-1 transition-transform duration-300">
          <div
            class="relative overflow-hidden aspect-video bg-white dark:bg-gray-800 flex items-center justify-center">
            <img
              :src="pengurusList[0].gambarUrl && pengurusList[0].gambarUrl.length > 0 ? resolveAssetUrl(pengurusList[0].gambarUrl[0]) : 'https://cdn-icons-png.flaticon.com/512/3135/3135715.png'"
