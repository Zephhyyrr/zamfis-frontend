<template>
    <BaseModal :modelValue="modelValue" @update:modelValue="$emit('update:modelValue', $event)" title="Hapus Permanen Jenis Kas"
        icon="lucide:alert-octagon" type="danger" :confirmText="'Hapus Permanen'" @confirm="submitDeletePermanent" @cancel="resetForm">
        ...
        <div class="bg-red-50 dark:bg-red-900/20 p-3 rounded-lg border border-red-100 dark:border-red-900 mb-4">
            <p class="text-xs text-red-800 dark:text-red-400 font-medium">
                Peringatan: Jika Anda menghapus data secara permanen, semua data yang terkait dengan <strong>{{ item?.nama }}</strong> akan ikut terhapus dan tidak dapat dipulihkan kembali.
            </p>
        </div>
        <p class="text-sm text-gray-700 dark:text-white mb-2">
            Ketik <strong>HAPUS {{ item?.nama }}</strong> untuk mengonfirmasi:
        </p>
        <input v-model="confirmationText" type="text" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 bg-white dark:bg-gray-800 text-gray-900 dark:text-white" :placeholder="`HAPUS ${item?.nama}`" />
        ...
        <template #footer>
            ...
            <BaseButton @click="submitDeletePermanent" text="Hapus Permanen"
                variant="danger" :fullWidth="false" :disabled="confirmationText !== `HAPUS ${item?.nama}`" />
        </template>
    </BaseModal>
</template>
...
const { deletePermanentKas } = useJenisKas();
