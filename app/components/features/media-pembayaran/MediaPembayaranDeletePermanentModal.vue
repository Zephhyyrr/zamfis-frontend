<template>
    <BaseModal :modelValue="modelValue" @update:modelValue="$emit('update:modelValue', $event)"
        title="Hapus Permanen Media Pembayaran" icon="lucide:alert-octagon" type="danger"
        :confirmText="'Hapus Permanen'" @confirm="submitDeletePermanent" @cancel="resetForm">
        ...
        <div class="bg-red-50 dark:bg-red-900/20 p-3 rounded-lg border border-red-100 dark:border-red-900 mb-4">
            <p class="text-xs text-red-800 dark:text-red-400 font-medium">
                Peringatan: Jika Anda menghapus data secara permanen, semua data yang terkait dengan <strong>{{
                    item?.nama }}</strong> akan ikut terhapus dan tidak dapat dipulihkan
                kembali.
            </p>
        </div>
...
import type { IMediaPembayaran } from '~/domain/models/IMediaPembayaran';

const props = defineProps<{
    modelValue: boolean;
    item: IMediaPembayaran | null;
}>();

const emit = defineEmits(['update:modelValue', 'success']);
const { deletePermanentMediaPembayaran } = useMediaPembayaran();
